...
def test_sklearn_with_preprocessing():
    ...
    wrapper = CIAFModelWrapper(
        ...
        model_name="SklearnTextClassifier",
...
def test_numerical_sklearn():
    ...
    ciaf_training_data = [
        {"content": json.dumps(X[i].tolist()), "metadata": {"id": f"{i}", "target": y[i]}}
...
def main():
    ...
    try:
        results.append(("Sklearn Text (Preprocessed)", test_sklearn_with_preprocessing()))
