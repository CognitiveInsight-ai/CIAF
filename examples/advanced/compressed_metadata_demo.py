...
def generate_sample_metadata(size: int = 1000) -> dict:
    ...
    return {
        ...
        "performance_metrics": {
            ...
            "confusion_matrix": [[random.randint(10, 100) for _ in range(3)] for _ in range(3)]
        },
        ...
        "training_history": {
            ...
            "accuracy": [random.uniform(0.7, 0.99) for _ in range(random.randint(10, 50))]
        },
        "environment": {
        ...
        "compliance_info": {
            ...
            "documentation_links": [f"https://docs.example.com/doc_{i}" for i in range(5)]
        },
        "large_data_array": [random.random() for _ in range(size)],  # Variable size data
...
def benchmark_storage_performance():
    ...
    for data_size in data_sizes:
        ...
        json_storage = MetadataStorage(
            storage_path=f"benchmark_json_{data_size}",
            backend="json"
        )
        ...
        json_size = sum(
            f.stat().st_size for f in Path(f"benchmark_json_{data_size}").rglob("*.json")
        ) / (1024 * 1024)  # MB
        ...
        print(f"    ✅ JSON: Write {json_write_time:.3f}s, Read {json_read_time:.3f}s, Size {json_size:.2f}MB")
        for comp, result in compression_results.items():
            print(f"    ✅ {comp.upper()}: Write {result['write_time']:.3f}s, "
    ...
    for data_size, result in results.items():
        ...
        print(f"  • Write: {json_result['write_time']:.3f}s ({json_result['write_time']/num_records*1000:.1f}ms/record)")
        print(f"  • Read:  {json_result['read_time']:.3f}s ({json_result['read_time']/num_records*1000:.1f}ms/record)")
        ...
        print(f"  • Write: {best_result['write_time']:.3f}s ({best_result['write_time']/num_records*1000:.1f}ms/record)")
        print(f"  • Read:  {best_result['read_time']:.3f}s ({best_result['read_time']/num_records*1000:.1f}ms/record)")
        print(f"  • Size:  {best_result['compressed_size_mb']:.2f}MB")
        print(f"  • Space Saved: {best_result['space_saved_mb']:.2f}MB ({best_result['compression_ratio']*100:.1f}%)")
...
def demonstrate_migration():
    ...
    json_storage = MetadataStorage(
        storage_path="demo_json_metadata",
        backend="json"
    )
    ...
    if migrated_count > 0:
        ...
        print(f"📏 Original size: {stats['total_uncompressed_size'] / (1024*1024):.2f} MB")
        print(f"🗜️ Compressed size: {stats['total_compressed_size'] / (1024*1024):.2f} MB")
    else:
