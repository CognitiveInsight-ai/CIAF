...
def demo_ml_framework_integration():
    ...
    data_secrets = {
        item["metadata"]["id"]: f"secret_{item['metadata']['id']}"
        for item in training_data
    }
...
def demo_complete_ciaf_workflow():
    ...
    mak = framework.create_model_aggregation_key(
        model_name="CIAFJobClassifier", authorized_datasets=[dataset_id]
    )
    ...
    print(
        f"🔍 Training integrity validation: {'✅ PASSED' if is_valid else '❌ FAILED'}"
    )
...
def demo_inference_receipts_and_chaining():
    ...
    print(
        f"🔍 Chain integrity verification: {'✅ VALID' if chain_valid else '❌ INVALID'}"
    )
...
def demo_model_wrapper():
    ...
    test_queries = ["Engineering job requirements", "Marketing salary range"]
...
def run_comprehensive_demo():
    ...
    print("=" * 60)
    print(
        "This demo showcases the reorganized CIAF framework with modular architecture"
    )
