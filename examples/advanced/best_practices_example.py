...
def create_sample_dataset(n_samples=1000, add_bias=True):
    ...
    base_score = (age * 0.02 + experience * 0.3 + education_score * 0.4 + 
    ...
    if add_bias:
        ...
        # Inject racial bias
        race_bias = np.where(race == 'White', 0.2, 
                   np.where(race == 'Asian', 0.1, -0.2))
...
def demonstrate_best_practices():
    ...
    print(f"⚖️ Bias Detected: {validation_result['bias_analysis']['attributes_analyzed']} attributes")
    print(f"🚀 Ready for Training: {'✅ Yes' if validation_result['ready_for_training'] else '❌ No'}")
    ...
    if validation_result['bias_analysis']['bias_detected']:
        ...
        for result in validation_result['bias_analysis']['results']:
            if result['bias_detected']:
                print(f"  • {result['protected_attribute']}: {result['bias_score']:.3f} "
    ...
    print(f"📊 Testing set: {len(X_test)} samples ({len(X_test)/len(dataset)*100:.1f}%)")
    
    # Step 5: Initialize Dataset Anchor with Phase Tracking
    print("\n📦 Step 5: Initialize Dataset Anchor with Phase Tracking")
    anchor = DatasetAnchor(
        dataset_id="best_practices_demo",
        model_name="hiring_model_best_practices"
    )
    ...
    anchor.set_phase_totals(
        training_total=len(X_train),
        testing_total=len(X_test)
    )
    ...
    for idx in capsulated_test_samples:
        anchor.add_data_item(
            ...
            metadata={"sample_id": idx, "phase": "testing", "prediction": int(y_pred[X_test.index.get_loc(idx)])},
            phase="testing"
        )
    ...
    if 'capsulation_status' in capsulation_summary:
        ...
        for phase, data in status.items():
            if isinstance(data, dict) and 'total' in data:
                print(f"    {phase}: {data.get('capsulated', 0)}/{data['total']} samples")
    ...
    metadata_id = metadata_manager.log_event(
        ...
        metadata={
            'pre_validation': {
                ...
                'bias_detected': validation_result['bias_analysis']['attributes_analyzed'],
