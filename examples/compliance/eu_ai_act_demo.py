...
from ciaf.compliance import ComplianceFramework, ComplianceTracker
...
def main():
    ...
    metadata_manager = ModelMetadataManager(
        model_id="hiring_ai_system_v1", storage_backend="json"
    )
    ...
    for i, pred in enumerate(predictions[:3]):
        print(
            f"     Candidate {i+1}: {'Recommended' if pred == 1 else 'Not Recommended'}"
        )
    ...
    compliance_tracker.track_eu_ai_act_compliance(
        risk_category="high", documentation_level="complete", monitoring_active=True
    )
    ...
    try:
        ...
        coverage = mapper.get_framework_coverage(
            ComplianceFramework.EU_AI_ACT, metadata
        )

        print(
            f"📈 EU AI Act Compliance Score: {coverage['overall_coverage']['coverage_percentage']:.1f}%"
        )
        print(
            f"📋 Requirements Satisfied: {coverage['overall_coverage']['satisfied_requirements']}/{coverage['overall_coverage']['total_requirements']}"
        )
