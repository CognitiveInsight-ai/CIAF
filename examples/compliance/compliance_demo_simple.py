...
def demo_regulatory_mapping():
    ...
    for framework, stats in framework_stats.items():
        coverage = (
            (stats["automated"] / stats["total"]) * 100 if stats["total"] > 0 else 0
        )
        print(
            f"  • {framework}: {stats['automated']}/{stats['total']} ({coverage:.1f}% automated)"
        )
...
def demo_framework_coverage():
    ...
    overall_coverage = (
        (total_automated / total_requirements) * 100 if total_requirements else 0
    )
