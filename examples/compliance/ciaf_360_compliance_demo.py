...
from ciaf.compliance import (  # Core modules; New enhanced modules
    ActionType,
    AuditTrailGenerator,
...
def demo_360_degree_compliance():
    ...
    print(
        f"   📈 Confidence Interval: [{uncertainty_metrics.confidence_interval.lower_bound:.3f}, {uncertainty_metrics.confidence_interval.upper_bound:.3f}] @ 95%"
    )
    print(
        f"   🔬 Method: {uncertainty_metrics.method.value} ({uncertainty_metrics.iterations} iterations)"
    )
    ...
    receipt = quantifier.generate_uncertainty_receipt(
        "PRED_20250802_001", uncertainty_metrics, "sha256_job_posting_input_hash"
    )
    ...
    uncertainty_metadata = quantifier.export_uncertainty_metadata(
    ...
    bias_action = action_logger.create_action(
        ...
        priority="High",
        evidence_files=[
            "fairness_audit_before_after.pdf",
            "bias_metrics_Q3_2025.json",
            "dataset_expansion_report.pdf",
        ],
        verification_criteria=[
            "Fairness score > 0.85",
            "Bias score < 0.05",
            "Performance maintained > 0.90",
        ],
    ...
    print(
        f"   ✅ Action Completed: Effectiveness {verification_results['fairness_score_after']:.1%}"
    )
    print(
        f"   🔗 Linked to Training Snapshot: {bias_action.linked_training_snapshot[:16]}..."
    )
    ...
    vulnerable_groups = impact_engine.register_stakeholder_group(
        ...
        vulnerability_factors=[
            ...
            "Legal protection dependency",
        ],
    )

    print(
        f"   👥 Registered: {job_seekers.name} ({job_seekers.size_estimate:,} individuals)"
    )
    print(
        f"   🛡️  Registered: {vulnerable_groups.name} ({vulnerable_groups.size_estimate:,} individuals)"
    )
    ...
    fairness_impact = impact_engine.create_impact_assessment(
        ...
        likelihood=0.35,
        potential_benefits=[
            "Consistent job categorization",
            "Reduced human bias",
            "Improved matching accuracy",
        ],
        potential_harms=[
            "Algorithmic discrimination",
            "Reduced opportunities for affected groups",
    ...
    print(
        f"   🎯 Overall Risk Level: {comprehensive_assessment.overall_risk_level.value}"
    )
    print(
        f"   👥 Stakeholder Groups: {len(comprehensive_assessment.stakeholder_groups)}"
    )
    print(
        f"   🗳️  Public Consultation: {comprehensive_assessment.public_consultation['participants']} participants"
    )

    stakeholder_metadata = impact_engine.create_compliance_metadata(
    ...
    print(
        f"   📊 Nodes: {viz_data['metadata']['node_count']} (Dataset Anchors, Model Checkpoints, Compliance Events)"
    )
    print(
        f"   🔗 Edges: {viz_data['metadata']['edge_count']} (Provenance Links, Audit Trails)"
    )
    ...
    for framework in security_frameworks[:3]:  # Show top 3
        ...
        print(
            f"   {framework.value}: {compliance['status']} ({compliance['compliance_rate']:.1%})"
        )
    ...
    print(
        f"   📋 Total Frameworks: {integrated_metadata['ciaf_compliance_framework']['frameworks_covered']}"
    )
    ...
    print(
        "   • EU AI Act: Article 9, 13, 15 (Risk Management, Transparency, Documentation)"
    )
