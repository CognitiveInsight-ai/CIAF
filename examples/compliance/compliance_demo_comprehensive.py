    ...
    for i in range(200):
        audit_generator.record_inference_event(
            ...
            explanation_data={
                ...
                "top_features": ["feature_1", "feature_2", "feature_3"] if i % 3 == 0 else None
            }
        )
...
def demo_risk_assessment(audit_generator):
    ...
    top_risks = sorted(assessment.risk_factors, key=lambda x: x.risk_score, reverse=True)[:3]
    print(f"\n🔥 Top Risk Factors:")
    for risk in top_risks:
        print(f"  • {risk.name}")
        print(f"    └─ Score: {risk.risk_score:.1f}, Impact: {risk.impact.value}, Likelihood: {risk.likelihood.value}")
        print(f"    └─ Mitigation: {risk.mitigation_measures[0] if risk.mitigation_measures else 'None specified'}")
    ...
    if assessment.security_assessment:
        print(f"\n🔒 Security Assessment:")
        print(f"  Security Score: {assessment.security_assessment.security_score:.1f}/100")
        print(f"  Vulnerabilities Found: {assessment.security_assessment.vulnerability_scan_results['vulnerabilities_found']}")
        print(f"  Adversarial Robustness: {assessment.security_assessment.adversarial_robustness['overall_robustness']:.1%}")
...
def demo_transparency_reporting(audit_generator, risk_engine):
    ...
    for report in transparency_generator.get_generated_reports():
        ...
        print(f"    └─ Audience: {report.target_audience.value.replace('_', ' ').title()}")
        print(f"    └─ Transparency Level: {report.transparency_level.value.title()}")
        print(f"    └─ Explainability Coverage: {report.algorithmic_metrics.explainability_coverage:.1%}")
    ...
    print(f"  Explainability Coverage: {dashboard_data['transparency_metrics']['explainability_coverage']:.1%}")
    print(f"  Reports Generated: {dashboard_data['reporting_status']['reports_generated']}")
    print(f"  Compliance Status: {dashboard_data['reporting_status']['compliance_status'].upper()}")
...
def main():
    ...
    try:
        ...
        doc_generator = demo_documentation_generation(validator, audit_generator, risk_engine)
        
        # Step 6: Demonstrate transparency reporting
        transparency_generator = demo_transparency_reporting(audit_generator, risk_engine)
        ...
        print(f"  Transparency Reports: {len(transparency_generator.get_generated_reports())}")
