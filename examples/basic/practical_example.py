...
class SimpleNumericalModel:
    ...
    def fit(self, X, y):
        ...
        y = np.array(y)
        self.weights = (
            np.random.random(X.shape[1]) if len(X.shape) > 1 else np.random.random(1)
        )
        self.is_fitted = True
    ...
    def predict(self, X):
        ...
        if len(X.shape) == 1:
            X = X.reshape(-1, 1)
        return (
            (X @ self.weights.reshape(-1, 1)).flatten()
            if len(self.weights) > 1
            else X.flatten() * self.weights
        )
...
class TextClassifierWithPreprocessing:
    ...
    def fit(self, X, y):
        ...
        self.is_fitted = True
        print(
            f"   TextClassifier fitted with {len(X)} samples, learned {len(self.word_scores)} words"
        )
...
def test_numerical_model():
    ...
    wrapper = CIAFModelWrapper(
        model=model, model_name="SimpleNumerical", enable_chaining=True
    )
...
def test_text_model_with_preprocessing():
    ...
    training_data = [
        {
            "content": "This is great excellent amazing",
            "metadata": {"id": "1", "target": 1},
        },
        {
            "content": "This is bad terrible horrible",
            "metadata": {"id": "2", "target": 0},
        },
        {"content": "Good quality nice product", "metadata": {"id": "3", "target": 1}},
        {
            "content": "Poor service disappointing experience",
            "metadata": {"id": "4", "target": 0},
        },
        {
            "content": "Outstanding fantastic wonderful",
            "metadata": {"id": "5", "target": 1},
        },
    ]

    model = TextClassifierWithPreprocessing()
    wrapper = CIAFModelWrapper(
        model=model, model_name="TextClassifier", enable_chaining=True
    )
...
def test_simple_callable():
    ...
    wrapper = CIAFModelWrapper(
        model=simple_predictor, model_name="SimpleCallable", enable_chaining=True
    )
...
def main():
    ...
    try:
        results.append(
            ("Text Model (Preprocessed)", test_text_model_with_preprocessing())
        )
    except Exception as e:
