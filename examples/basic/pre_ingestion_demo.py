...
def generate_biased_dataset(n_samples=1000):
    ...
    gender = np.random.choice(
        ["Male", "Female", "Non-binary"], n_samples, p=[0.6, 0.35, 0.05]
    )
    ...
    bias_multiplier = np.where(
        gender == "Male",
        1.2,  # 20% bias toward males
        np.where(gender == "Female", 0.9, 1.0),
    )  # 10% bias against females

    race_bias = np.where(
        race == "White",
        1.1,  # 10% bias toward white candidates
        np.where(np.isin(race, ["Black", "Hispanic"]), 0.85, 1.0),
    )  # 15% bias against minorities

    # Calculate hiring probability with bias
    base_score = (
        0.3 * education_score / 10
        + 0.4 * np.minimum(experience_years / 10, 1.0)
        + 0.3 * skill_assessment / 10
    )
...
def demonstrate_pre_ingestion_validation():
    ...
    if validation_report["bias_analysis"]["bias_detected"]:
        ...
        for result in validation_report["bias_analysis"]["results"]:
            if result["bias_detected"]:
                print(
                    f"  • {result['protected_attribute']}: {result['bias_score']:.3f} bias score"
                )
    ...
    if not validation_report["ready_for_training"]:
        ...
        critical_issues = [
            issue
            for issue in validation_report["validation_issues"]["details"]
...
def demonstrate_capsulation_tracking():
    ...
    print(
        f"  Training set: {len(X_train)} samples ({len(X_train)/len(data_clean)*100:.1f}%)"
    )
    print(
        f"  Testing set: {len(X_test)} samples ({len(X_test)/len(data_clean)*100:.1f}%)"
    )
    ...
    test_sample_size = int(
        len(indices_test) * 0.8
    )  # Simulate 80% of test samples processed
    print(
    ...
    for phase, status in summary["capsulation_status"].items():
        if status["total"] > 0:
            ...
            print(
                f"    Capsulated: {status['capsulated']}/{status['total']} ({status['percentage']:.1f}%)"
            )
