...
from ..provenance import TrainingSnapshot, ProvenanceCapsule, ModelAggregationKey
...
try:
    from ..explainability import CIAFExplainer, explainability_manager, create_auto_explainer
...
try:
    from ..uncertainty import CIAFUncertaintyQuantifier, uncertainty_manager, create_auto_quantifier
...
try:
    from ..metadata_tags import CIAFMetadataTag, tag_generator, create_classification_tag, CIAFTagEncoder
...
class CIAFModelWrapper:
    ...
    def train(
        ...
        print(f"🚀 [{self.model_name}] Starting verifiable training for version '{model_version}'...")
        ...
        try:
            ...
            if fit_model and hasattr(self.model, 'fit'):
                ...
                if X is not None:
                    ...
                    print(f"⚠️  [{self.model_name}] Could not extract X,y - skipping model.fit()")
            ...
            print(f"🎯 [{self.model_name}] Training snapshot: {self.training_snapshot.snapshot_id}")
    ...
    def predict(
        ...
        print(f"🔮 [{self.model_name}] Running verifiable inference (v{model_version})...")
        ...
        try:
            ...
            if use_model and hasattr(self.model, 'predict'):
                try:
                    ...
                    print(f"⚠️  [{self.model_name}] Model prediction failed: {model_error}")
            ...
            raise RuntimeError(f"Prediction failed for {self.model_name}: {str(e)}") from e
    ...
    def verify(self, receipt: InferenceReceipt) -> Dict[str, Any]:
        ...
        if self.training_snapshot:
            verification_results["snapshot_integrity"] = self.framework.validate_training_integrity(self.training_snapshot)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.training_snapshot:
            info.update({
                "training_snapshot_id": self.training_snapshot.snapshot_id,
                "training_data_count": len(self.training_snapshot.provenance_capsule_hashes)
            })
    ...
    def _validate_training_data(self, training_data: List[Dict[str, Any]]) -> None:
        ...
        for i, item in enumerate(training_data):
            ...
            if 'metadata' not in item or not isinstance(item['metadata'], dict):
                raise ValueError(f"Training data item {i} missing or invalid 'metadata' field")
            
            if 'id' not in item['metadata']:
                raise ValueError(f"Training data item {i} metadata missing 'id' field")

    def _prepare_model_data(self, training_data: List[Dict[str, Any]]) -> Tuple[Any, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"   🔧 Using enhanced preprocessing for {type(self.model).__name__}")
                
                # Pass the training data directly to auto_preprocess_data
                # Also pass self to store the fitted preprocessor
                X_processed, y_processed = auto_preprocess_data(training_data, store_preprocessor=self)
                
                if X_processed is not None:
                    print(f"   ✅ Auto-preprocessing successful - X shape: {getattr(X_processed, 'shape', len(X_processed))}")
                    return X_processed, y_processed
                else:
                    print(f"   ⚠️ Auto-preprocessing failed")
                    
            except ImportError:
                print(f"   ⚠️ Enhanced preprocessing not available - using legacy method")
                ...
                if X and isinstance(X[0], str):
                    ...
                    if any(sklearn_name in model_name for sklearn_name in ['Regression', 'Forest', 'Bayes', 'SVM', 'Classifier']):
                        print(f"   Detected text data with sklearn-like model ({model_name})")
                        print(f"   Note: Text data needs vectorization for sklearn models")
                        print(f"   Consider using TfidfVectorizer or similar preprocessing")
                ...
                if y is not None:
                    try:
                        ...
                        X = np.array(X) if all(isinstance(x, (int, float)) for x in X) else X
                        y = np.array(y)
    ...
    def _predict_with_model(self, query: Union[str, List, Any]) -> Any:
        ...
        if hasattr(self.model, 'predict'):
            try:
                ...
                if isinstance(query, str) and self.preprocessing_type == 'text' and self.fitted_vectorizer:
                ...
                elif not isinstance(query, str) and self.preprocessing_type == 'numerical' and self.fitted_preprocessor:
                    ...
                    if isinstance(query, (list, tuple)):
                        ...
                        query_array = np.array(query).reshape(-1, 1)  # Each value is a separate sample
                    else:
                        # Single value
                        query_array = np.array([[query]])  # Single sample, single feature
                    ...
                    return prediction if len(prediction) > 1 else (prediction[0] if len(prediction) > 0 else prediction)
    ...
    def _extract_top_features(self, query: str, top_k: int = 3) -> list:
        ...
        try:
            ...
            positive_words = ['excellent', 'great', 'fantastic', 'wonderful', 'amazing', 'outstanding', 'superior']
            negative_words = ['terrible', 'awful', 'horrible', 'poor', 'bad', 'disappointing', 'waste']
