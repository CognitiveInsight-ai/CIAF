...
class ComprehensiveStakeholderImpactAssessment:
    ...
    def get_impacts_by_severity(self, severity: ImpactSeverity) -> List[ImpactAssessment]:
        """Get all impacts of a specific severity."""
        return [impact for impact in self.impact_assessments if impact.severity == severity]
    
    def get_impacts_by_stakeholder(self, stakeholder_group_id: str) -> List[ImpactAssessment]:
        """Get all impacts for a specific stakeholder group."""
        return [impact for impact in self.impact_assessments if impact.stakeholder_group_id == stakeholder_group_id]
    
    def get_high_risk_impacts(self) -> List[ImpactAssessment]:
        """Get all high and critical severity impacts."""
        return [
            impact for impact in self.impact_assessments 
...
class StakeholderImpactAssessmentEngine:
    ...
    def register_stakeholder_group(
        ...
        group_id = f"SH_{stakeholder_type.name}_{len(self.stakeholder_registry) + 1:03d}"
    ...
    def create_impact_assessment(
        ...
        assessment_id = f"IA_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{len(self.assessments):03d}"
    ...
    def get_latest_assessment(self) -> Optional[ComprehensiveStakeholderImpactAssessment]:
    ...
    def generate_assessment_summary(
        ...
        for severity in ImpactSeverity:
            severity_counts[severity.value] = len(assessment.get_impacts_by_severity(severity))
        
        # Count impacts by category
        category_counts = {}
        for category in ImpactCategory:
            category_impacts = [i for i in assessment.impact_assessments if i.impact_category == category]
        ...
        if assessment.impact_assessments:
            avg_likelihood = sum(i.likelihood for i in assessment.impact_assessments) / len(assessment.impact_assessments)
            avg_confidence = sum(i.confidence_level for i in assessment.impact_assessments) / len(assessment.impact_assessments)
        else:
            avg_likelihood = 0.0
            avg_confidence = 0.0
        ...
        return {
            ...
            "critical_impacts": len(assessment.get_impacts_by_severity(ImpactSeverity.CRITICAL)),
            "high_impacts": len(assessment.get_impacts_by_severity(ImpactSeverity.HIGH)),
    ...
    def create_compliance_metadata(
        self,
        assessment: Optional[ComprehensiveStakeholderImpactAssessment] = None
    ) -> Dict[str, Any]:
        ...
        return {
            "stakeholder_impact_assessment": {
                ...
                "summary": {
                    ...
                    "public_summary": {
                        ...
                        "public_consultation_results": {
                            "consultation_period": assessment.public_consultation.get("period", "Not conducted") if assessment.public_consultation else "Not conducted",
                            "total_participants": assessment.public_consultation.get("participants", 0) if assessment.public_consultation else 0,
                            "vulnerable_group_participation": assessment.public_consultation.get("vulnerable_group_participation", 0) if assessment.public_consultation else 0,
                            "overall_sentiment": assessment.public_consultation.get("feedback_summary", "No feedback available") if assessment.public_consultation else "No consultation conducted",
                ...
                "external_documentation": {
                    ...
                    "public_consultation_report": f"public_consultation_{assessment.assessment_date[:7]}.pdf" if assessment.public_consultation else None,
...
def demo_stakeholder_impact_assessment():
    ...
    job_seekers = engine.register_stakeholder_group(
        ...
        size_estimate=50000,
        demographic_info={"age_range": "18-65", "primary_regions": ["North America", "Europe"]},
        vulnerability_factors=["Unemployment status", "Economic disadvantage", "Digital literacy gaps"]
    )
    ...
    vulnerable_groups = engine.register_stakeholder_group(
        ...
        size_estimate=15000,
        vulnerability_factors=["Historical discrimination", "Systemic bias", "Limited advocacy resources"]
    )
    ...
    fairness_impact = engine.create_impact_assessment(
        ...
        potential_harms=["Algorithmic discrimination", "Reduced job opportunities for affected groups"],
        mitigation_measures=[
        ...
        evidence_sources=["Bias audit report Q2 2025", "Academic literature on job classification bias"]
    )
    ...
    privacy_impact = engine.create_impact_assessment(
        ...
        monitoring_indicators=["Data collection metrics", "User consent rates", "Privacy complaints"],
