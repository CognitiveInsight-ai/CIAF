...
class CorrectiveActionLogger:
    ...
    def create_action(
        ...
        action_id = f"CAL_{datetime.now().strftime('%Y%m%d')}_{str(uuid.uuid4())[:8].upper()}"
    ...
    def generate_action_summary(
        ...
        for action_type in ActionType:
            by_type[action_type.value] = len([a for a in actions if a.action_type == action_type])
        
        # Count by trigger
        by_trigger = {}
        for trigger_type in TriggerType:
            by_trigger[trigger_type.value] = len([a for a in actions if a.trigger_type == trigger_type])
        ...
        if completed_actions:
            ...
            for action in completed_actions:
                created = datetime.fromisoformat(action.date_created.replace('Z', '+00:00'))
                applied = datetime.fromisoformat(action.date_applied.replace('Z', '+00:00'))
        ...
        return {
            "corrective_action_log": {
                ...
                "summary": {
                    ...
                    "avg_effectiveness": sum(summary.effectiveness_scores) / len(summary.effectiveness_scores) if summary.effectiveness_scores else 0.0
                },
                "recent_actions": [
                    {
                        "action_id": action.action_id,
                        ...
                        "cost": {
                            ...
                            "currency": "USD",
                            "cost_breakdown": {
                                "data_collection": (action.actual_cost or action.cost_estimate or 0.0) * 0.4,
                                "model_retraining": (action.actual_cost or action.cost_estimate or 0.0) * 0.4,
                                "validation_testing": (action.actual_cost or action.cost_estimate or 0.0) * 0.2
                            },
                            "budget_variance": (action.actual_cost or 0.0) - (action.cost_estimate or 0.0),
                            "budget_variance_percentage": (((action.actual_cost or 0.0) - (action.cost_estimate or 0.0)) / (action.cost_estimate or 1.0)) * 100 if action.cost_estimate else 0.0
                        }
                    }
                    for action in sorted(self.actions, key=lambda x: x.date_created, reverse=True)[:5]
                ],
...
def demo_corrective_action_log():
    ...
    if summary.effectiveness_scores:
        avg_effectiveness = sum(summary.effectiveness_scores) / len(summary.effectiveness_scores)
