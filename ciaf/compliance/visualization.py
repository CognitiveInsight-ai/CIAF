...
class CIAFVisualizationEngine:
    ...
    def create_provenance_node(
        ...
        if position is None:
            ...
            count_offset = len([n for n in self.node_registry.values() if n.node_type == node_type]) * 20
        ...
        node = VisualizationNode(
            ...
            size=1.5 if node_type in [NodeType.DATASET_ANCHOR, NodeType.MODEL_CHECKPOINT] else 1.0
        )
    ...
    def create_3d_provenance_visualization(
        ...
        config = VisualizationConfig(
            ...
            export_formats=[ExportFormat.GLTF, ExportFormat.JSON_GRAPH, ExportFormat.WEBGL],
            interactive_features=[
                "node_selection", "edge_filtering", "time_navigation", 
                "compliance_highlighting", "audit_trail_playback"
            ],
        ...
        dataset_node = self.create_provenance_node(
            ...
            {"dataset_size": 100000, "data_source": "multiple_platforms", "anchor_hash": "0x123..."},
            position=(0, 0, 0)
        )
        ...
        model_node = self.create_provenance_node(
            ...
            {"architecture": "transformer", "parameters": 150000000, "training_completed": True},
            position=(100, 50, 0)
        )
    ...
    def create_compliance_metadata(self) -> Dict[str, Any]:
        ...
        if self.visualizations:
            latest_viz_id = max(self.visualizations.keys(), key=lambda x: self.visualizations[x]['metadata']['creation_timestamp'])
        ...
        return {
            "visualization": {
                ...
                "viewer_url": f"https://ciaf-visualizer.demo/graph/{self.model_name.lower().replace(' ', '_')}" if latest_viz else None,
                "accessibility": {
                    "wcag_compliant": True,
                    "keyboard_navigation": True,
                    "screen_reader_support": True,
                ...
                "metadata": {
                    ...
                    "latest_visualization": latest_viz['config']['visualization_id'] if latest_viz else None,
                    "last_updated": latest_viz['metadata']['creation_timestamp'] if latest_viz else None
                },
