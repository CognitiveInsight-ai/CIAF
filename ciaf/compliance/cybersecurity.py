...
class SecurityControlImplementation:
    ...
    def __post_init__(self):
        ...
        if self.next_assessment_date is None and self.last_assessment_date:
            last_date = datetime.fromisoformat(self.last_assessment_date.replace('Z', '+00:00'))
...
class CybersecurityAssessment:
    ...
    def get_compliance_by_framework(self, framework: SecurityFramework) -> Dict[str, Any]:
        """Get compliance status for a specific framework."""
        framework_controls = [c for c in self.control_implementations if c.framework == framework]
...
class CybersecurityComplianceEngine:
    ...
    def _initialize_control_catalog(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "IAM_001": {
                ...
                "requirements": ["MFA enabled", "Backup authentication methods", "Regular review"]
            },
            "ENC_001": {
                ...
                "requirements": ["AES-256 encryption", "Key management", "Encrypted backups"]
            },
            "ENC_002": {
            ...
            "LOG_001": {
                ...
                "requirements": ["Complete event logging", "Log integrity", "Long-term retention"]
            },
            "VUL_001": {
                ...
                "requirements": ["Automated scanning", "Manual testing", "Remediation tracking"]
            },
            "INC_001": {
                ...
                "requirements": ["Response procedures", "Contact lists", "Regular testing"]
            },
            "PII_001": {
                "name": "PII Protection",
                ...
                "requirements": ["PII identification", "Access controls", "Data minimization"]
            }
        }
    ...
    def assess_control_compliance(
        ...
        control_implementation.last_assessment_date = datetime.now(timezone.utc).isoformat()
    ...
    def conduct_cybersecurity_assessment(
        ...
        for framework in frameworks:
            framework_controls = [
                control_id for control_id, info in self.control_catalog.items()
        ...
        compliant_controls = len([c for c in control_implementations if c.status == ComplianceStatus.COMPLIANT])
        ...
        non_compliant = [c for c in control_implementations if c.status != ComplianceStatus.COMPLIANT]
        if non_compliant:
            findings.append({
                "finding_id": "SEC_001",
                "severity": "Medium",
            ...
            recommendations.append("Address non-compliant security controls within 30 days")
        
        if overall_score < 0.9:
            recommendations.append("Implement additional security monitoring capabilities")
    ...
    def create_compliance_metadata(
        self,
        assessment: Optional[CybersecurityAssessment] = None
    ) -> Dict[str, Any]:
        ...
        return {
            "cybersecurity_compliance": {
                ...
                "iso_27001_ready": SecurityFramework.ISO_27001 in assessment.frameworks_assessed,
                "soc2_type2_ready": SecurityFramework.SOC2_TYPE2 in assessment.frameworks_assessed,
                ...
                "security_controls": {
                    ...
                    "compliant": len([c for c in assessment.control_implementations if c.status == ComplianceStatus.COMPLIANT]),
                    "non_compliant": len([c for c in assessment.control_implementations if c.status == ComplianceStatus.NON_COMPLIANT]),
                    "in_progress": len([c for c in assessment.control_implementations if c.status == ComplianceStatus.IN_PROGRESS])
                },
                ...
                "findings_summary": {
                    ...
                    "critical": len([f for f in assessment.findings if f.get("severity") == "Critical"]),
                    "high": len([f for f in assessment.findings if f.get("severity") == "High"]),
                    "medium": len([f for f in assessment.findings if f.get("severity") == "Medium"])
                },
                "remediation": {
                ...
                "integration_capabilities": {
                    ...
                    "compliance_dashboards": ["ServiceNow GRC", "MetricStream", "LogicGate"]
                },
        ...
        if format == "summary":
            return {
                "executive_summary": {
                    ...
                    "risk_level": assessment.risk_level,
                    "frameworks_assessed": [f.value for f in assessment.frameworks_assessed],
...
def demo_cybersecurity_compliance():
    ...
    for framework in frameworks:
        ...
        print(f"     Controls: {compliance['compliant']}/{compliance['controls']} ({compliance['compliance_rate']:.1%})")
    ...
    print(f"   Vulnerability Scanners: {', '.join(integration['vulnerability_scanners'])}")
    print(f"   Compliance Dashboards: {', '.join(integration['compliance_dashboards'])}")
