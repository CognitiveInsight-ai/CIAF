...
from .regulatory_mapping import RegulatoryMapper, ComplianceFramework, ComplianceRequirement
from .audit_trails import AuditTrailGenerator, ComplianceAuditRecord, AuditEventType
...
class ComplianceValidator:
    ...
    def validate_data_governance(
        ...
        data_events = audit_generator.get_audit_trail(
            start_date, end_date, [AuditEventType.DATA_ACCESS, AuditEventType.DATA_INGESTION]
        )
        ...
        result = ValidationResult(
            ...
            message="Data access events are being logged" if data_events else "No data access events found",
            details={"data_events_count": len(data_events)},
            evidence=[f"Found {len(data_events)} data access events"],
            recommendations=[] if data_events else ["Ensure data access logging is enabled"],
        ...
        result = ValidationResult(
            ...
            message=f"Found {len(pii_events)} events with PII data" if pii_events else "No PII data events detected",
            details={"pii_events_count": len(pii_events)},
            evidence=[f"PII events: {[e.event_id for e in pii_events[:5]]}"] if pii_events else ["No PII detected"],
        ...
        result = ValidationResult(
            ...
            details={"encryption_rate": encryption_rate, "total_events": len(data_events)},
            evidence=[f"Encryption used in {len(encrypted_events)}/{len(data_events)} events"],
            recommendations=["Enable encryption for all data operations"] if encryption_rate < 0.95 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
    ...
    def validate_audit_integrity(
        ...
        result = ValidationResult(
            ...
            message="Audit trail integrity verified" if integrity_check["integrity_verified"] else "Audit trail integrity compromised",
            details=integrity_check,
            evidence=["Cryptographic integrity verification passed"] if integrity_check["integrity_verified"] else ["Integrity verification failed"],
            recommendations=[] if integrity_check["integrity_verified"] else ["Investigate audit trail corruption"],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        result = ValidationResult(
            ...
            recommendations=["Restore audit trail from backup"] if broken_chains > 0 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        result = ValidationResult(
            ...
            recommendations=["Review system clock synchronization"] if timestamp_issues > 0 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
    ...
    def validate_access_controls(
        ...
        anonymous_events = [e for e in all_events if not e.user_id or e.user_id == "anonymous"]
        ...
        result = ValidationResult(
            ...
            evidence=[f"User identification: {len(user_events)}/{len(all_events)} events"],
            recommendations=["Implement user authentication for all operations"] if anonymous_rate >= 0.1 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        result = ValidationResult(
            ...
            details={"control_rate": control_rate, "controlled_events": len(events_with_controls)},
            evidence=[f"Access controls: {len(events_with_controls)}/{len(all_events)} events"],
            recommendations=["Implement access controls for all operations"] if control_rate < 0.9 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        privileged_events = [e for e in high_risk_events if "admin" in str(e.access_controls).lower()]
        ...
        result = ValidationResult(
            ...
            severity=ValidationSeverity.MEDIUM,
            status="pass" if len(privileged_events) == 0 or len(privileged_events) < len(high_risk_events) * 0.5 else "warning",
            message=f"Privileged access events: {len(privileged_events)}",
            details={"privileged_events": len(privileged_events), "high_risk_events": len(high_risk_events)},
            evidence=[f"Privileged access monitoring: {len(privileged_events)} events"],
            recommendations=["Review privileged access procedures"] if len(privileged_events) > 0 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
    ...
    def validate_risk_management(
        ...
        risk_events = [e for e in all_events if e.event_type == AuditEventType.RISK_ASSESSMENT]
        ...
        result = ValidationResult(
            ...
            recommendations=["Conduct regular risk assessments"] if not risk_events else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        result = ValidationResult(
            ...
            recommendations=["Review high-risk event handling procedures"] if len(high_risk_events) > 10 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
        ...
        high_risk_rate = risk_distribution["high"] / len(all_events) if all_events else 0
        ...
        result = ValidationResult(
            ...
            recommendations=["Review risk classification criteria"] if high_risk_rate > 0.2 else [],
            timestamp=datetime.now(timezone.utc).isoformat()
        )
        results.append(result)
            ...
            for req in requirements:
                if req.documentation_required:
                    result = ValidationResult(
                        ...
                        severity=ValidationSeverity.MEDIUM if req.mandatory else ValidationSeverity.LOW,
                        ...
                        recommendations=[f"Prepare {doc}" for doc in req.documentation_required],
                        timestamp=datetime.now(timezone.utc).isoformat()
                    )
                    results.append(result)
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        for severity in ValidationSeverity:
            severity_counts[severity.value] = len([r for r in self.validation_results if r.severity == severity])
        ...
        for result in self.validation_results:
            ...
            if framework not in framework_counts:
                framework_counts[framework] = {"pass": 0, "fail": 0, "warning": 0, "total": 0}
        ...
        return {
            ...
            "failing": failing,
            "warnings": warnings,
            "pass_rate": (passing / total_validations) * 100 if total_validations else 0,
    ...
    def get_validations_by_severity(self, severity: ValidationSeverity) -> List[ValidationResult]:
