...
class PreIngestionValidator:
    ...
    def validate_dataset(self, 
        ...
        """
        print(f"🔍 Starting pre-ingestion validation for dataset with {len(data)} samples...")
    ...
    def _detect_bias(self, data: pd.DataFrame, target_column: str, protected_attributes: List[str]):
        ...
        for protected_attr in protected_attributes:
            ...
            bias_result = self._analyze_bias_for_attribute(data, target_column, protected_attr)
            self.bias_results.append(bias_result)
            
            if bias_result.bias_detected:
                severity = ValidationSeverity.ERROR if bias_result.bias_score > 0.3 else ValidationSeverity.WARNING
    ...
    def _analyze_bias_for_attribute(self, data: pd.DataFrame, target_column: str, 
                                  protected_attr: str) -> BiasDetectionResult:
        """Analyze bias for a specific protected attribute."""
        try:
            # Calculate outcome rates by group
            group_outcomes = data.groupby(protected_attr)[target_column].agg(['mean', 'count'])
            ...
            bias_detected = bias_score > 0.1 and p_value < 0.05  # 10% difference threshold
            
            # Identify most and least advantaged groups
            sorted_groups = outcome_rates.sort_values(ascending=False)
            affected_groups = [f"{group}: {rate:.3f}" for group, rate in sorted_groups.items()]
            ...
            affected_groups = [f"{group}: {rate:.3f}" for group, rate in outcome_rates.items()]
    ...
    def _validate_privacy_compliance(self, data: pd.DataFrame, sensitive_columns: List[str]):
        ...
        for column in numeric_columns:
            ...
            if len(col_data) > 0:
                ...
                if outlier_percentage > 5:
                    severity = ValidationSeverity.WARNING if outlier_percentage < 15 else ValidationSeverity.ERROR
    ...
    def _validate_compliance_requirements(self, data: pd.DataFrame, protected_attributes: List[str]):
    ...
    def _generate_validation_report(self, data: pd.DataFrame, validation_duration: float) -> Dict[str, Any]:
        ...
        if quality_score < 50:
            ...
            overall_recommendation = "⚠️ Dataset needs attention - address critical issues before proceeding"
        elif quality_score < 85:
            overall_recommendation = "✅ Dataset is acceptable with minor issues to address"
        else:
        ...
        return {
            ...
            "bias_analysis": {
                ...
                "bias_detected": any(result.bias_detected for result in self.bias_results),
                "results": [
                    {
            ...
            "ready_for_training": quality_score >= 70 and not any(
                issue.severity == ValidationSeverity.CRITICAL for issue in self.validation_issues
            )
        }
    ...
    def _print_validation_summary(self, report: Dict[str, Any]):
        ...
        print(f"📊 Dataset: {report['dataset_summary']['total_samples']:,} samples, "
...
def validate_before_split(data: pd.DataFrame, 
                         target_column: str,
                         protected_attributes: List[str],
...
if __name__ == "__main__":
    ...
    print("Run this module with your dataset for comprehensive validation before train/test split")
