...
class MetadataStorage:
    ...
    def _get_model_sqlite(self, model_name: str, stage: Optional[str], limit: int) -> List[Dict[str, Any]]:
    ...
    def _get_model_files(self, model_name: str, stage: Optional[str], limit: int) -> List[Dict[str, Any]]:
    ...
    def add_compliance_event(self, 
        ...
        if self.backend == "sqlite":
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (event_id, metadata_id, framework, compliance_score, validation_status, timestamp, details))
        ...
        if model_name:
            ...
            for model_dir in self.storage_path.iterdir():
                if model_dir.is_dir() and model_dir.name != "compliance_events":
                    metadata_records.extend(self.get_model_metadata(model_dir.name, limit=1000))
