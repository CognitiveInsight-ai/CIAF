    ...
    def fit(self, X_train: np.ndarray, training_data: Optional[List[Dict]] = None) -> 'CIAFExplainer':
        ...
        try:
            if self.method == ExplanationMethod.SHAP_TREE and SHAP_AVAILABLE:
                ...
                warnings.warn(f"Explanation method {self.method} not available or not supported")
...
class CIAFExplainabilityManager:
    ...
    def register_explainer(self, 
                          model_id: str,
                          model: Any,
                          method: str = ExplanationMethod.SHAP_KERNEL,
                          feature_names: Optional[List[str]] = None) -> CIAFExplainer:
        """Register an explainer for a model."""
        explainer = CIAFExplainer(model, method, feature_names)
        self.explainers[model_id] = explainer
        return explainer
    
    def fit_explainer(self, model_id: str, X_train: np.ndarray, training_data: Optional[List[Dict]] = None) -> bool:
    ...
    def explain_prediction(self, 
        ...
        base_explanation = {
            ...
            "input_shape": X.shape if hasattr(X, 'shape') else len(X) if hasattr(X, '__len__') else 1,
        }
...
def create_auto_explainer(model: Any, feature_names: Optional[List[str]] = None) -> CIAFExplainer:
    """Create automatically selected explainer based on model type."""
    # Auto-detect best explainer method
    if hasattr(model, 'feature_importances_'):
        # Tree-based model
        method = ExplanationMethod.SHAP_TREE if SHAP_AVAILABLE else ExplanationMethod.FEATURE_IMPORTANCE
    elif hasattr(model, 'coef_'):
        # Linear model
        method = ExplanationMethod.SHAP_LINEAR if SHAP_AVAILABLE else ExplanationMethod.FEATURE_IMPORTANCE
    else:
        # General model
        method = ExplanationMethod.SHAP_KERNEL if SHAP_AVAILABLE else ExplanationMethod.LIME_TABULAR
