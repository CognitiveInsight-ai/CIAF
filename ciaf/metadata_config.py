...
class MetadataConfig:
    ...
    DEFAULT_CONFIG = {
        ...
        "required_metadata_fields": [
            "model_name", "stage", "event_type", "timestamp"
        ],
    ...
    def validate(self) -> Dict[str, str]:
        ...
        if self.config["compliance_retention_days"] < self.config["metadata_retention_days"]:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "create-template":
            ...
            output = sys.argv[3] if len(sys.argv) > 3 else f"ciaf_config_{template}.json"
