...
class CIAFUncertaintyQuantifier:
    ...
    def fit(self, X_cal: Optional[np.ndarray] = None, y_cal: Optional[np.ndarray] = None) -> 'CIAFUncertaintyQuantifier':
        """
    ...
    def _monte_carlo_dropout(self, X: np.ndarray) -> UncertaintyEstimate:
        ...
        for _ in range(self.n_samples):
            ...
            if hasattr(self.model, 'predict_proba'):
                ...
                noise = np.random.normal(0, abs(pred) * 0.05) if pred != 0 else np.random.normal(0, 0.1)
        ...
        total_uncertainty = float(np.mean(std_pred)) if hasattr(std_pred, '__len__') else float(std_pred)
        ...
        if hasattr(mean_pred, '__len__') and len(mean_pred) > 1:
            prediction = mean_pred[1] if len(mean_pred) > 1 else mean_pred[0]  # Positive class for classification
            interval = (float(lower[1]) if len(lower) > 1 else float(lower[0]), 
                       float(upper[1]) if len(upper) > 1 else float(upper[0]))
        else:
            prediction = float(mean_pred) if hasattr(mean_pred, '__len__') else mean_pred
    ...
    def _prediction_intervals(self, X: np.ndarray) -> UncertaintyEstimate:
        ...
        if hasattr(self.model, 'predict_proba'):
            ...
            probs = self.model.predict_proba(X)[0] if hasattr(self.model.predict_proba(X), '__len__') else [0.5, 0.5]
...
class CIAFUncertaintyManager:
    ...
    def register_quantifier(self,
                           model_id: str,
                           model: Any,
                           method: UncertaintyMethod = UncertaintyMethod.MONTE_CARLO_DROPOUT,
                           n_samples: int = 100) -> CIAFUncertaintyQuantifier:
    ...
    def predict_with_uncertainty(self, model_id: str, X: np.ndarray) -> Optional[UncertaintyEstimate]:
...
def create_monte_carlo_quantifier(model: Any, n_samples: int = 100) -> CIAFUncertaintyQuantifier:
    """Create Monte Carlo Dropout uncertainty quantifier."""
    return CIAFUncertaintyQuantifier(model, UncertaintyMethod.MONTE_CARLO_DROPOUT, n_samples)

def create_bootstrap_quantifier(model: Any, n_samples: int = 50) -> CIAFUncertaintyQuantifier:
    """Create Bootstrap uncertainty quantifier."""
    return CIAFUncertaintyQuantifier(model, UncertaintyMethod.BOOTSTRAP_AGGREGATION, n_samples)
