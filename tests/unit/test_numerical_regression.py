    ...
    try:
        ...
        for i in range(10):
            ...
            numerical_data.append(
                {"content": x, "metadata": {"id": str(i), "target": y}}
            )

        print(f"Training data: {len(numerical_data)} samples")
        print(
            f"Sample: x={numerical_data[0]['content']}, y={numerical_data[0]['metadata']['target']}"
        )
        ...
        for i, test_input in enumerate(test_cases, 1):
            ...
            if hasattr(receipt, "enhanced_info") and receipt.enhanced_info:
                ...
                if "uncertainty" in info:
                    print(
                        f"     uncertainty: {info['uncertainty']['confidence_level']}"
                    )
